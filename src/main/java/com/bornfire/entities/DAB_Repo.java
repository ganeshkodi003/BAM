...
public interface DAB_Repo extends CrudRepository<DAB_Entity, String> {
	...
	@Query(value = "SELECT * FROM BAM_DAILY_ACCT_BAL WHERE ACCT_NUM IN (:accountNumbers)", nativeQuery = true)
	...
	@Query(value = "SELECT gl_desc AS primary_gl_desc, gl_code, gl_desc AS secondary_gl_desc, glsh_code, glsh_desc, COUNT(GLSH_CODE) as sum, acct_crncy, "
			...
			+ "FROM BAM_DAILY_ACCT_BAL " + "WHERE :balancedate BETWEEN TRAN_DATE AND END_TRAN_DATE "
...
List<Object[]> getAccountBalancesByGlshCode(@Param("formattedDate") String formattedDate, @Param("glshCode") String glshCode);
	...
	@Query(value = "SELECT  GLSH_CODE, GLSH_DESC,COUNT(GLSH_CODE) as sum, acct_crncy, SUM(tran_date_bal) FROM BAM_DAILY_ACCT_BAL WHERE gl_desc='Asset' AND :balancedate BETWEEN TRAN_DATE AND END_TRAN_DATE GROUP BY GLSH_CODE, GLSH_DESC, acct_crncy ORDER BY GLSH_CODE ASC", nativeQuery = true)
	List<Object[]> getfilteredrec(@Param("balancedate") Date balancedate);

	@Query(value = "SELECT  GLSH_CODE, GLSH_DESC,COUNT(GLSH_CODE) as sum, acct_crncy, SUM(tran_date_bal) FROM BAM_DAILY_ACCT_BAL WHERE gl_desc='Liability' AND :balancedate BETWEEN TRAN_DATE AND END_TRAN_DATE GROUP BY GLSH_CODE, GLSH_DESC, acct_crncy ORDER BY GLSH_CODE ASC", nativeQuery = true)
	List<Object[]> getfilteredrec1(@Param("balancedate") Date balancedate);

	@Query(value = "SELECT * FROM BAM_DAILY_ACCT_BAL WHERE gl_desc='Income' AND :balancedate BETWEEN TRAN_DATE AND END_TRAN_DATE", nativeQuery = true)
	List<DAB_Entity> getfilteredrec2(@Param("balancedate") Date balancedate);

	@Query(value = "SELECT * FROM BAM_DAILY_ACCT_BAL WHERE gl_desc='Expense' AND :balancedate BETWEEN TRAN_DATE AND END_TRAN_DATE", nativeQuery = true)
	List<DAB_Entity> getfilteredrec3(@Param("balancedate") Date balancedate);

	@Query(value = "SELECT SUM(TRAN_CR_BAL) as TRANAMT, SUM(TRAN_DR_BAL) as TRANAMT1 FROM BAM_DAILY_ACCT_BAL", nativeQuery = true)
	Object[] getcheck3();

	@Query(value = " select a.customer_id, a.loan_accountno,a.customer_name,a.date_of_loan,a.loan_sanctioned ,b.TRAN_DATE_BAL  from Loan_AccountMaster a , BAM_DAILY_ACCT_BAL b where\r\n"
	...
	@Query(value = " select a.cust_id, a.depo_actno,a.cust_name,a.deposit_date,a.deposit_amt, a.deposit_period,a.rate_of_int,a.maturity_date ,b.TRAN_DATE_BAL  from Loan_AccountMaster a , BAM_DAILY_ACCT_BAL b where\r\n"
