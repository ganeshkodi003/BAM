...
@Query(value = "SELECT COUNT(TRAN_AMT) AS COUNTAMT, SUM(TRAN_AMT) AS TRANAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND PART_TRAN_TYPE = 'debit' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(SYSDATE)", nativeQuery = true)
List<Object[]> getWoforDebitValues();

	
@Query(value = "SELECT COUNT(TRAN_AMT) AS COUNTAMT, SUM(TRAN_AMT) AS TRANAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND PART_TRAN_TYPE = 'credit' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(SYSDATE)", nativeQuery = true)
List<Object[]> getWoforcreditValues();


	
@Query(value = "SELECT COUNT(DISTINCT acct_num) AS COUNTACCTNUM,COUNT(TRAN_AMT) AS COUNTAMT,SUM(TRAN_AMT) AS TRANAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(SYSDATE)", nativeQuery = true)
Object[] getWoforTotalValues();



@Query(value = "SELECT acct_num,acct_name,COUNT(TRAN_AMT) AS COUNTAMT,SUM(CASE WHEN PART_TRAN_TYPE = 'credit' THEN TRAN_AMT ELSE 0 END) AS TOTAL_CREDIT, SUM(CASE WHEN PART_TRAN_TYPE = 'debit' THEN TRAN_AMT ELSE 0 END) AS TOTAL_DEBIT, SUM(CASE WHEN PART_TRAN_TYPE = 'credit' THEN TRAN_AMT WHEN PART_TRAN_TYPE = 'debit' THEN -TRAN_AMT ELSE 0 END) AS NETAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(TO_DATE(:tranDate, 'YYYY-MM-DD')) GROUP BY acct_num, acct_name",  // Group by acct_name as well
        nativeQuery = true)
...
@Query(value = "SELECT SUM(CASE WHEN PART_TRAN_TYPE = 'credit' THEN TRAN_AMT ELSE 0 END) - SUM(CASE WHEN PART_TRAN_TYPE = 'debit' THEN TRAN_AMT ELSE 0 END) AS NET_TOTAL FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(TO_DATE(:tranDate, 'YYYY-MM-DD'))", nativeQuery = true)
...
@Query(value = "SELECT SUM(COUNTAMT) AS TOTAL_COUNT FROM ( SELECT COUNT(TRAN_AMT) AS COUNTAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(TO_DATE(:tranDate, 'YYYY-MM-DD')) GROUP BY acct_num) AccountCounts", nativeQuery = true)
...
@Query(value = "SELECT SUM(TRAN_AMT * -1) AS TOTAL_DEBIT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND PART_TRAN_TYPE = 'debit' AND TRAN_STATUS = 'POSTED' AND TRUNC(TRAN_DATE) = TRUNC(TO_DATE(:tranDate, 'YYYY-MM-DD'))", 
        nativeQuery = true)
BigDecimal getTotalDebit(@Param("tranDate") String tranDate);



@Query(value = "select  * from BAM_TRM_WRK_TRANSACTIONS where  acct_num IN (:acctnum) ", nativeQuery = true)
List<TRAN_MAIN_TRM_WRK_ENTITY> set_dab_acc_num(@Param("acctnum") List<String> acctnum);
/*till this Barath*/
	
	@Query(value = "SELECT COUNT(TRAN_AMT) as COUNTAMT,SUM(TRAN_AMT) AS TRANAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG !='Y' AND PART_TRAN_TYPE='debit' AND TRAN_STATUS='POSTED'", nativeQuery = true)
	Object[] getwofordebitvalues();

	@Query(value = "SELECT COUNT(TRAN_AMT) as COUNTAMT,SUM(TRAN_AMT) AS TRANAMT FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG !='Y' AND PART_TRAN_TYPE='credit' AND TRAN_STATUS='POSTED'", nativeQuery = true)
	Object[] getwoforcreditvalues();

	@Query(value = "SELECT COUNT(TRAN_AMT) AS COUNTAMT, SUM(CASE WHEN PART_TRAN_TYPE = 'Debit' THEN -1 * TRAN_AMT  WHEN PART_TRAN_TYPE = 'Credit' THEN 1 * TRAN_AMT ELSE 0 END) AS TRANAMT  FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG != 'Y' AND TRAN_STATUS = 'POSTED'", nativeQuery = true)
	Object[] getTransactionValues();
	
	@Query(value = "SELECT COUNT(PART_TRAN_TYPE) as TRAN_TYPE FROM BAM_TRM_WRK_TRANSACTIONS WHERE DEL_FLG='Y'", nativeQuery = true)
	Object getdelvalues();

	@Query(value = "SELECT COUNT(DISTINCT(TRAN_ID)) as ID FROM BAM_TRM_WRK_TRANSACTIONS WHERE TRAN_STATUS='ENTERED'", nativeQuery = true)
	...
	@Query(value = "select  * from BAM_TRM_WRK_TRANSACTIONS order by TRAN_DATE,TRAN_ID,PART_TRAN_ID ", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> findByjournal();
	
	@Query(value = "select  * from BAM_TRM_WRK_TRANSACTIONS where tran_date=?1 order by TRAN_DATE,TRAN_ID,PART_TRAN_ID  ", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> findByjournal1(Date date);
	
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS aa where aa.part_tran_id =?2 and aa.tran_id =?1 ", nativeQuery = true)
	TRAN_MAIN_TRM_WRK_ENTITY getmodifyjournal(String tran_id, String part_tran_id);
	
	@Query(value = "select  * from BAM_TRM_WRK_TRANSACTIONS where tran_id=?1", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> findByjournalmodify();
	
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS aa where aa.part_tran_id =?2 and aa.tran_id =?1 ", nativeQuery = true)
	TRAN_MAIN_TRM_WRK_ENTITY getValuepop(String tran_id, String part_tran_id);
	
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS aa where aa.part_tran_id =?3 and aa.tran_id =?1 and aa.acct_num = ?2 ", nativeQuery = true)
	...
	@Query(value = "select tran_id,Part_tran_id from BAM_TRM_WRK_TRANSACTIONS aa where aa.acct_num = ?1", nativeQuery = true)
	TRAN_MAIN_TRM_WRK_ENTITY gettranpopvalues(String acct_num);
	
	@Query(value = "select tran_amt from BAM_TRM_WRK_TRANSACTIONS WHERE part_tran_type = 'Credit' AND TRAN_ID = ?1 AND PART_TRAN_ID=?2", nativeQuery = true)
	BigDecimal getcredit(String tran_id,BigDecimal partTranID);
	
	@Query(value = "select tran_amt from BAM_TRM_WRK_TRANSACTIONS WHERE part_tran_type = 'Debit' AND TRAN_ID = ?1 AND PART_TRAN_ID=?2", nativeQuery = true)
	BigDecimal getdebit(String tran_id,BigDecimal partTranID);
	
	@Query(value = "SELECT TRAN_DATE,TRAN_ID || '/' || TO_CHAR(PART_TRAN_ID) AS TRANSACTION_ID,TRAN_PARTICULAR,ACCT_CRNCY,TRAN_AMT,CASE WHEN PART_TRAN_TYPE = 'credit' THEN TRAN_AMT ELSE 0 END AS Credit,CASE WHEN PART_TRAN_TYPE = 'debit' THEN TRAN_AMT ELSE 0 END AS Debit FROM BAM_TRM_WRK_TRANSACTIONS WHERE ACCT_NUM = '1500001000' AND TRAN_STATUS = 'POSTED' ORDER BY TRAN_DATE, TRAN_ID, PART_TRAN_ID", nativeQuery = true)
	List<Object[]> getList(@Param("acct_num") String acct_num);
	
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS aa where aa.acct_num = ?1", nativeQuery = true)
	TRAN_MAIN_TRM_WRK_ENTITY gettranpopvaluesdata(String acct_num);
	
	@Query(value = "SELECT MAX(part_tran_id) AS part_tran_id FROM BAM_TRM_WRK_TRANSACTIONS WHERE tran_id =?1", nativeQuery = true)
	String  maxPartranID(String tranId);
	
	@Query(value = "SELECT MIN(part_tran_id) AS part_tran_id FROM BAM_TRM_WRK_TRANSACTIONS WHERE tran_id = ?1", nativeQuery = true)
	String minPartTranID(String tranId);
	
	@Query(value = "SELECT COUNT(DISTINCT part_tran_id) FROM BAM_TRM_WRK_TRANSACTIONS WHERE tran_id = ?1", nativeQuery = true)
	Integer countPartTranIDs(String tranId);

	@Query(value = "SELECT DISTINCT part_tran_id FROM BAM_TRM_WRK_TRANSACTIONS WHERE tran_id = ?1", nativeQuery = true)
...
List<TRAN_MAIN_TRM_WRK_ENTITY> getTranList(String acctNum, String fromDate, String toDate);
	...
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS WHERE acct_num =?1 AND TRAN_STATUS='POSTED' ORDER BY TRAN_DATE", nativeQuery = true)
	...
	@Query(value = "SELECT " +
            ...
            "FROM BAM_TRM_WRK_TRANSACTIONS WHERE TRAN_STATUS= 'POSTED'", 
    nativeQuery = true)
Object[] getcheck1();

	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS WHERE TRAN_DATE=?1 AND ENTRY_USER=?2 AND ACCT_NUM LIKE '%LA%' ORDER BY TRAN_ID", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> getTransactionRecords(Date con_date,String user);
	
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS WHERE TRAN_DATE=?1", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> getvalueusingdate(Date date);
	
	
	
	@Query(value = "select * from BAM_TRM_WRK_TRANSACTIONS WHERE TRAN_DATE=?1 AND tran_status='POSTED'", nativeQuery = true)
