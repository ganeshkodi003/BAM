...
public interface GeneralLedgerRep extends CrudRepository<GeneralLedgerEntity,String> {
	...
	@Query(value = "SELECT * from BAM_GENERAL_LED WHERE del_flg='N' ", nativeQuery = true)
	List<GeneralLedgerEntity> getRefCodelist();
	
	@Query(value = "SELECT * from BAM_GENERAL_LED WHERE GL_CODE=?1 ", nativeQuery = true)
	GeneralLedgerEntity getRefMaster(String GL_CODE);
	
	@Query(value = "SELECT * from BAM_GENERAL_LED ", nativeQuery = true)
	List<GeneralLedgerEntity> getlistvalue();
	
	@Query(value = "SELECT * from BAM_GENERAL_LED WHERE GLSH_CODE=?1", nativeQuery = true)
	GeneralLedgerEntity getsinglevalue(String glsh_Code);

	@Modifying
	@Transactional
	@Query(value = "UPDATE BAM_GENERAL_LED " +
